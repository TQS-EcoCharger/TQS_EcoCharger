...
class ChargingSessionServiceImplTest {
    ...
    void testStartSessionWithOtp_success() {
        ...
        when(reservationRepo.findFirstByChargingPointIdAndStartTimeBeforeAndEndTimeAfter(eq(pointId), any(), any()))
            .thenReturn(Optional.of(reservation));
        when(otpRepo.findByCodeAndReservation(eq(otp), eq(reservation))).thenReturn(Optional.of(otpCode));
    ...
    void testStartSessionWithOtp_invalidOtp_throwsException() {
        ...
        when(reservationRepo.findFirstByChargingPointIdAndStartTimeBeforeAndEndTimeAfter(anyLong(), any(), any()))
            .thenReturn(Optional.of(reservation));
        when(otpRepo.findByCodeAndReservation(any(), eq(reservation)))
            .thenReturn(Optional.empty());

        assertThrows(IllegalArgumentException.class, () -> service.startSessionWithOtp(pointId, "invalid", carId));
    }
}

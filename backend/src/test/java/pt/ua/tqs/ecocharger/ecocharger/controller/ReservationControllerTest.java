...
import co.elastic.clients.elasticsearch.connector.Connector;
...
public class ReservationControllerTest {
    ...
    void setUp() {
        ...
        ChargingPointDTO chargingPoint = new ChargingPointDTO(2L, "ChargingBrand", true, 100.0, 10.0, connectors, chargingStation);
        reservationResponse = new ReservationResponseDTO(1L, 1L, chargingPoint, LocalDateTime.parse("2023-05-28T10:00:00"), LocalDateTime.parse("2023-05-28T12:00:00"), ReservationStatus.PENDING);
        ...
        when(reservationService.getActiveReservationsByChargingPointId(2L)).thenReturn(List.of(reservationResponse));
        when(reservationService.createReservation(any(ReservationRequestDTO.class))).thenReturn(reservationResponse);
    }
    ...
    void testCreateReservation() throws Exception {
        mockMvc.perform(post("/api/v1/reservation")
                        ...
                        .content("{\"userId\":1, \"chargingPointId\":2, \"startTime\":\"2023-05-28T10:00:00\", \"endTime\":\"2023-05-28T12:00:00\"}"))
    ...
    @Test
    @DisplayName("Getting active reservations by charging point ID returns 200 OK with reservations list")
    void testGetActiveReservationsByChargingPoint() throws Exception {
        mockMvc.perform(get("/api/v1/reservation/point/2/active"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))

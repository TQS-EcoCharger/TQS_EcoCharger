   ...
   @Test
    ...
    void testCreateAdministrator() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/admin")
            ...
            .andExpect(jsonPath("$.password").value("pass1")); // Se retornar senha (não recomendado em produção)
    }
    @Test
    @DisplayName("Update a charging station")
    @Requirement("ET-20")
    void testUpdateChargingStation() throws Exception {
        ChargingStation updatedStation = new ChargingStation("Lisboa", "Rua A", 38.7169, -9.1399, "PT", "Portugal");
        ...
        mockMvc.perform(put("/api/v1/admin/stations/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isOk())
    ...
    void testDeleteChargingStation() throws Exception {
        ChargingStation station = new ChargingStation("Porto", "Rua B", 41.1496, -8.6109, "PT", "Portugal");
        station.setId(1L);
        when(administratorService.deleteChargingStation(1L)).thenReturn(station);
        mockMvc.perform(delete("/api/v1/admin/stations/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())

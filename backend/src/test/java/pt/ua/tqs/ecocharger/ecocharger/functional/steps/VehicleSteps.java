...
public class VehicleSteps {
  ...
  public void the_table_should_have_the_following_columns(
    ...
    for (int i = 0; i < expectedColumns.size(); i++) {
      assertEquals(
          ...
          headerCells.get(i).getText().trim(),
    ...
    WebElement button =
        wait.until(
            ExpectedConditions.elementToBeClickable(
                By.cssSelector(
                    "button." + buttonClass + "[data-vehicle-name='" + vehicleName + "']")));
    button.click();
  }

  @Then("the table should contain the following data:")
  public void the_table_should_contain_the_following_data(
      io.cucumber.datatable.DataTable dataTable) {
    ...
    for (int i = 0; i < expectedData.size(); i++) {
      ...
      for (String key : expectedRow.keySet()) {
        ...
        assertEquals(
            expectedRow.get(key),
            cells.get(columnIndex).getText().trim(),
  ...
  public void i_click_on_the_button(String buttonClass) {
    WebElement button =
        wait.until(ExpectedConditions.elementToBeClickable(By.className(buttonClass)));
    button.click();
  }
  ...
  public void i_fill_in_the_vehicle_form_with(io.cucumber.datatable.DataTable dataTable) {
    ...
    for (Map.Entry<String, String> entry : data.entrySet()) {
      WebElement field =
          wait.until(ExpectedConditions.visibilityOfElementLocated(By.id(entry.getKey())));
      field.clear();
  ...
  public void i_submit_the_vehicle_form() {
    WebElement submitButton =
        wait.until(ExpectedConditions.elementToBeClickable(By.id("submit-vehicle")));
    submitButton.click();
  }
  ...
  public void the_table_should_have_rows(int expectedRowCount) {
    ...
    assertEquals(
        expectedRowCount,
        vehicleRows.size(),
        "Vehicle table does not have " + expectedRowCount + " rows");
  }
  ...
  public void i_click_on_the_button_for_the_vehicle_with_ID(String buttonId, int vehicleId) {
    WebElement button =
        wait.until(ExpectedConditions.elementToBeClickable(By.id(buttonId + "-" + vehicleId)));
    button.click();
  }
  ...
  public void i_should_see_the_vehicle_details_page_for(String vehicleName) {
    WebElement vehicleDetails =
        wait.until(
            ExpectedConditions.visibilityOfElementLocated(By.id("vehicle-details-container")));
    assertTrue(
        vehicleDetails.getText().contains(vehicleName),
        "Vehicle details page does not contain the expected vehicle name");
  }
  ...
  public void i_should_see_the_following_details(io.cucumber.datatable.DataTable dataTable) {
    ...
    for (Map<String, String> detail : expectedDetails) {
      for (Map.Entry<String, String> entry : detail.entrySet()) {
        ...
        assertEquals(
            expectedValue,
            detailElement.getText().trim(),
            "Detail " + id + " does not match expected value");
      }
    }
  }
}

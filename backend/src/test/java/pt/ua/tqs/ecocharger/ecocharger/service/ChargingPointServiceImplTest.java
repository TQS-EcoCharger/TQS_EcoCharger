...
class ChargingPointServiceImplTest {
    ...
    @InjectMocks
    private ChargingPointServiceImpl chargingPointService;
    ...
    void setUp() {
        ...
        station = new ChargingStation("Aveiro", "Rua A", 40.0, -8.0, "Rua A", "PT", "Portugal", "Electric");
        station.setId(1L);
    ...
    void testCreateChargingPoint_NewChargingPoint() {
        when(chargingPointRepository.findByChargingStationId(station.getId())).thenReturn(Optional.of(new ArrayList<>()));
    ...
    void testCreateChargingPoint_AlreadyExists() {
        ...
        when(chargingPointRepository.findByChargingStationId(station.getId())).thenReturn(Optional.of(existingPoints));
    ...
    void testGetAvailablePoints() {
        ...
        when(chargingPointRepository.findAvailablePointsByChargingStation(station)).thenReturn(Optional.of(availablePoints));
    ...
    void testGetAvailablePoints_Empty() {
        when(chargingPointRepository.findAvailablePointsByChargingStation(station)).thenReturn(Optional.empty());

        Exception exception = assertThrows(RuntimeException.class, () ->
                chargingPointService.getAvailablePoints(station));
    ...
    void testDeletePoint_NotFound() {
        ...
        Exception exception = assertThrows(RuntimeException.class, () ->
                chargingPointService.deletePoint(2L));
    ...
    void testGetPointsByStationId_NotFound() {
        ...
        Exception exception = assertThrows(RuntimeException.class, () ->
                chargingPointService.getPointsByStationId(1L));

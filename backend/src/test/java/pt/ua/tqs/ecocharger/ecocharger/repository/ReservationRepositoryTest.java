...
public class ReservationRepositoryTest {
    ...
    void testExistsByChargingPointIdAndTimeOverlap_shouldReturnTrueWhenOverlapping() {
        ...
        Reservation reservation = new Reservation(null, user, chargingPoint,
            now.plusHours(1), now.plusHours(2), ReservationStatus.PENDING);
    ...
    void testFindByChargingPointIdAndEndTimeAfter_shouldReturnFutureReservations() {
        ...
        Reservation reservation = new Reservation(null, user, chargingPoint,
            now.plusMinutes(30), now.plusHours(1), ReservationStatus.PENDING);
        reservationRepository.save(reservation);

        List<Reservation> results = reservationRepository.findByChargingPointIdAndEndTimeAfter(
            chargingPoint.getId(), now
        );

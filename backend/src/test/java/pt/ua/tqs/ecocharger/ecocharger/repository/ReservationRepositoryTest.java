...
public class ReservationRepositoryTest {
  ...
  void setup() {
    ...
    ChargingStation station =
        new ChargingStation(
            "Aveiro", "123 Main St", 40.6405, -8.6538, "Main St", "PT", "Portugal", "Electric");
    station = chargingStationRepository.save(station);

    // Create and save ChargingPoint
    chargingPoint = new ChargingPoint(station, true, "Tesla", Collections.emptyList());
  ...
  void testFindByChargingPointIdAndEndTimeAfter_shouldReturnFutureReservations() {
    ...
    List<Reservation> results =
        reservationRepository.findByChargingPointIdAndEndTimeAfter(chargingPoint.getId(), now);

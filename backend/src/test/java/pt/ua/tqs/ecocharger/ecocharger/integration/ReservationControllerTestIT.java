...
public class ReservationControllerTestIT {
    ...
    @Autowired
    private ChargingPointRepository pointRepository;
    ...
    void setUp() {
        ...
        ChargingStation station = stationRepository.save(new ChargingStation("City", "Addr", 1.0, 1.0, "Street", "CC", "Country", "EV"));
        point = pointRepository.save(new ChargingPoint(station, true, "BrandX", Collections.emptyList()));
    ...
    void testCreateReservationConflict() {
        Reservation r = new Reservation(null, user, point, LocalDateTime.now().plusHours(1), LocalDateTime.now().plusHours(2), ReservationStatus.CONFIRMED);
        ...
        """,
            user.getId(), point.getId(),
            LocalDateTime.now().plusMinutes(90), LocalDateTime.now().plusHours(3));
    ...
    void testGetReservationsByUserId() {
        RestAssuredMockMvc.given()
            .when().get("/api/v1/reservation/user/" + user.getId())
            .then().statusCode(200);
    }
    ...
    void testGetActiveReservationsByChargingPoint() {
        RestAssuredMockMvc.given()
            .when().get("/api/v1/reservation/point/" + point.getId() + "/active")
            .then().statusCode(200);
    }
}
